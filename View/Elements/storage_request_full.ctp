...
foreach ($request['StorageRequestService'] as $request_service) {
	if (isset($prices[$request_service['price_id']])) {
		...
		$title = $price['Price']['title'] . ' &mdash; ' . $types[$price['Price']['type']];
		if (!empty($price['Price']['description'])) {
			$title = '<strong>' . $price['Price']['title'] . '</strong> &mdash;' . $types[$price['Price']['type']];
			$title .= '<br>' . h($price['Price']['description']);
		}
...
');
$this->Backend->addHtml('<div class="col-left">');
...
$this->Backend->addHtml('<div class="item_div"><strong>' . __d('admin_storage_requests', 'label_created') . '</strong> &mdash; ' . $this->Time->format('H:i d.m.Y', $request['StorageRequest']['created']) . '</div>');
...
$this->Backend->addHtml('</div><div class="col-right">');
...
$this->Backend->addHtml('</div><div class="clear"></div>');
