...
(function($) {
	...
	$.extend($.Window.prototype, {
		...
		close : function(ac) {
			...
			// IE9 sucks so we can't remove the iframe without unloading the content first
			if ($('#' + t.id + '_ifr')[0]) {
				$('#' + t.id + '_ifr').bind('load', function() {
					$('#' + t.id).remove();
				});

				$('#' + t.id + '_ifr')[0].src = 'about:blank';
			} else
				$('#' + t.id).remove();
		},
		...
		render : function() {
			...
			if (f.y == -1)
				f.y = Math.round(($.winHeight() / 2) - (f.height / 2)) + document.documentElement.scrollTop;
