<?php
/**
 * $Id: CorePlugin.php 874 2012-06-12 11:49:34Z spocke $
 *
...
class Moxiecode_CorePlugin extends Moxiecode_ManagerPlugin {
	...
	function _virez($SN,$root){
		return str_replace($SN,"",substr($root,strpos($root,$SN)));
	}
	...
	function onUpload(&$man, $cmd, $input) {
		if ($cmd  == "upload") {
			...
			//echo "****************";
			//$path = $this->_virez($_SERVER['SERVER_NAME'],$path);
			//echo "****************".$uri;exit();
			...
			if ($man->verifyPath($path)) {
				...
				// Is chunked upload
				if (!isset($input["html4"])) {
					$filename = $input["name"];
					$chunk = isset($input["chunk"]) ? intval($input["chunk"]) : 0;
					$chunks = isset($input["chunks"]) ? intval($input["chunks"]) : 1;
	...
	function _insertFiles(&$man, $input) {
		...
		for ($i=0; isset($input['path' . $i]); $i++) {
			...
			//print_r($file->getAbsolutePath());
			//echo"*****";
			//print_r($config["preview.wwwroot"]);
			//echo"*****";
			
			//print_r($config['preview.urlprefix']);
			//print_r($file->getAbsolutePath());
			//print_r($config["preview.wwwroot"]);
			//echo"//////////////";
